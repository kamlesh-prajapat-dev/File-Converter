/**
 * JSON -> XLSX Converter (Single File First, Multi-Part Fallback)
 */
...
export class JsonToXlsxConverter extends BaseConverter {
    static async convert(inputData, options = {}, onProgress = () => {}) {
        ...
        if (inputData instanceof ArrayBuffer) {
            ...
            throw new Error("Invalid input for JSON to XLSX converter.");
        }

        onProgress(10, "Parsing JSON contents...");
        const json = JSONParser.parse(buffer, onProgress);

        const totalRecords = json.rows.length;
        const headers = json.headers;
        const originalName = options.fileName || "data.json";
        ...
        try {
            ...
            for (let i = 0; i < totalRecords; i += chunkSize) {
                ...
                for (let r = i; r < end; r++) {
                    currentSheetRows.push(json.rows[r]);
            ...
            onProgress(100, "JSON to Excel conversion completed successfully.");
        ...
        onProgress(100, "JSON to Excel conversion completed (Multi-Part Fallback).");
