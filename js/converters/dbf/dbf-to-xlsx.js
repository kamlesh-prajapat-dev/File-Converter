/**
 * DBF -> XLSX Converter (Proactive Memory-Safe Export Chunking)
 */
...
export class DBFToXlsxConverter extends BaseConverter {
    static async convert(inputData, options = {}, onProgress = () => {}) {
        ...
        // Safe XLSX limit per file (50,000 rows max per file)
        ...
        for (let partIndex = 0; partIndex < totalParts; partIndex++) {
            ...
            // Build AOA (Array of Arrays) for this safe 50,000 row chunk
            const chunkData = [headers];
            for (let i = start; i < end; i++) {
                chunkData.push(dbf.records[i]);
            }

            // Create fresh single workbook for this chunk
            const workbook = XLSX.utils.book_new();
            const worksheet = XLSX.utils.aoa_to_sheet(chunkData);
            XLSX.utils.book_append_sheet(workbook, worksheet, "Data");

            // Write XLSX ArrayBuffer safely without RangeError
            ...
            // Dereference temporary chunk data
            chunkData.length = 0;
        }
